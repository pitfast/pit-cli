...
type Route = { application_id: string; path: string; service_id: string; release_id: string };
...
function App() {
  ...
  return <div className="shell">
    ...
    <main><header><div><p className="eyebrow">PITFAST LOCAL</p><h1>{page}</h1></div><div className="status"><span className="dot" /> authority connected<br /><small>control plane · loopback</small></div></header>
...
function Routes({ data }: { data: Snapshot }) { return <PageIntro title="Routes / PitLane" copy="Logical paths resolve to ServiceId inside an ApplicationRelease. Services do not own listeners." panels={[<Panel title="Logical routing" key="routes"><RouteTable routes={data.routes} /></Panel>]} empty="No active routes." />; }
function RouteTable({ routes }: { routes: Array<{ application_id: string; path: string; service_id: string; release_id?: string }> }) { return <Table headers={["Application", "Path", "ServiceId", "Release"]}>{routes.map((route) => <tr key={`${route.application_id}-${route.path}`}><td>{route.application_id}</td><td className="mono">{route.path}</td><td className="accent">→ {route.service_id}</td><td className="mono">{short(route.release_id)}</td></tr>)}</Table>; }
